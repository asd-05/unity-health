...
// Utility to convert numbers to px
...
const useAnimationLoop = (
  ...
  }, [trackRef, seqWidth, targetVelocity, isHovered, pauseOnHover]);
};
...
export const LogoLoop: React.FC<LogoLoopProps> = memo(
  ...
  () =>
    ...
    } as React.CSSProperties), // <-- cast here
  [gap, logoHeight, fadeOutColor]
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: string) => {
        ...
        const itemAriaLabel = isNodeItem ? item.ariaLabel ?? item.title : item.ariaLabel ?? item.alt ?? item.title;

        const itemContent = item.href ? (
          <a className="logoloop__link" href={item.href} aria-label={itemAriaLabel} target="_blank" rel="noreferrer noopener">
            {content}
          </a>
        ) : (
          content
